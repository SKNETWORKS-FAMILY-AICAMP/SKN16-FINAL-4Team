...
### 🖥️ 허원준 - Sub PM / Backend Lead

백엔드 서버 구축과 배포 경험

본 프로젝트에서 백엔드 서버 구축과 배포를 담당했습니다. FastAPI 기반으로 핵심 API를 구현했으며, 로컬 개발 환경과 서버 환경의 차이를 고려해 배포 구조를 설계했습니다. 단순히 서버를 실행하는 데 그치지 않고, 실제 서비스 환경에서도 동일한 동작이 이루어지도록 구성하는 데 중점을 두었습니다.

 프론트엔드–백엔드 연동 문제 해결

배포 이후 프론트엔드에서 백엔드 API 호출이 정상적으로 이루어지지 않는 문제가 발생했습니다. 단편적인 설정 수정에 그치기보다는, 빌드 과정과 실행 환경, 서버 설정 등 전체 흐름을 차분히 점검하며 원인을 추적했습니다. 그 결과 환경에 따라 설정이 다르게 적용되고 있음을 확인했고, 구조를 정리해 회원가입과 로그인 기능이 서비스 환경에서도 안정적으로 동작하도록 개선했습니다.

 안정성과 운영 관점의 배움

이 과정을 통해 백엔드 개발자는 기능 구현에 그치지 않고, 배포 이후 발생하는 문제까지 책임지고 해결해야 한다는 점을 체감했습니다. 실제 사용자 관점에서 기능이 정상적으로 동작하는지 반복적으로 확인하며, 운영 환경을 고려한 개발의 중요성을 배울 수 있었습니다.

 아쉬웠던 점과 향후 개선 방향

배포 이후 서버 상태와 에러를 체계적으로 모니터링할 수 있는 환경을 충분히 구축하지 못한 점은 아쉬움으로 남았습니다. 앞으로는 로그 관리와 성능 모니터링 도구를 도입해 문제를 보다 빠르게 인지하고, 안정적인 서비스 운영이 가능한 백엔드 환경을 구축해 나가고자 합니다.

이번 프로젝트를 진행하면서 백엔드 서버 개발자로서 팔요한 역량을 기를 수 있었고 뿐만 아니라 팀원간 소통의 중요성을 다시 느낄 수 있는 정말 값진 경험을 한 것 같습니다. 
