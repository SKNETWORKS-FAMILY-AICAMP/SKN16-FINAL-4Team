...
# 세부 계절 라벨 생성
df['subseason'] = df['season'] + '_' + df['subtype']

print(f"\n전체 데이터: {len(df)}개")
print("\n클래스별 분포:")
for subseason in sorted(df['subseason'].unique()):
    count = (df['subseason'] == subseason).sum()
    print(f"  {subseason}: {count}개")

# Feature 준비
feature_cols = ['a_median', 'b_median', 'chroma', 'L_raw']

X = df[feature_cols].values
y = df['subseason'].values
...
for subseason in sorted(df['subseason'].unique()):
    ...
    total = mask.sum()
    print(f"  {subseason}: {correct}/{total} = {correct/total:.1%}")
...
model_data = {
    ...
    'cv_score': best_score,
    'train_acc': acc,
    'classes': sorted(df['subseason'].unique().tolist())
}

with open('full_subseason_ml_model.pkl', 'wb') as f:
    pickle.dump(model_data, f)

print(f"\n✅ 모델 저장: full_subseason_ml_model.pkl")
